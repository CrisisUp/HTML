...
// Adicione no início do arquivo, após as declarações de variáveis
...
// Modifique a função startGame para incluir o tema
function startGame() {
...
function aiMove() {
  ...
  // 1. Primeiro verifica capturas obrigatórias
  const captureMoves = [];
  for (let piece of blackPieces) {
    const moves = getValidMoves(piece, true);
    captureMoves.push(...moves.filter(m => m.isCapture));
  }
  ...
  // 2. Se não houver capturas, verifica movimentos que não travem a peça
  const safeMoves = [];
  for (let piece of blackPieces) {
    const moves = getValidMoves(piece, false);

    // Filtra movimentos que não deixem a peça travada
    const nonTrappingMoves = moves.filter(move => {
      // Simula o movimento
      ...
      // Verifica se após mover ainda terá movimentos
      const hasFutureMoves = getValidMoves(piece, false).length > 0;

      // Restaura posição original
      piece.dataset.row = originalRow;
      piece.dataset.col = originalCol;

      return hasFutureMoves;
    });

    safeMoves.push(...nonTrappingMoves);
  }

  // 3. Se houver movimentos seguros, escolhe um aleatório
  ...
  // 4. Se todos os movimentos levam ao travamento, escolhe um aleatório (último recurso)
  const allMoves = [];
  for (let piece of blackPieces) {
    allMoves.push(...getValidMoves(piece, false));
  }
...
// Nova função para obter todos os movimentos válidos para uma peça
function getValidMoves(piece, checkCapturesOnly) {
  const row = Number(piece.dataset.row);
  const col = Number(piece.dataset.col);
  const color = piece.dataset.color;
  const isKing = piece.dataset.king === 'true';
  ...
  // Se só queremos capturas, retorna agora
  if (checkCapturesOnly) return moves;
...
// Nova função para executar movimento da IA
function executeAIMove(move) {
  selected = move.piece;
  if (move.isCapture) {
    removePieceAt(move.capture[0], move.capture[1]);
  }
