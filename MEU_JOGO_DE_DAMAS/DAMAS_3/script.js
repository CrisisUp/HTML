...
// Variável para controlar o estado da assistência por voz
let voiceAssistEnabled = false;
const voiceCommands = {
  'mover': handleMoveCommand,
  'capturar': handleCaptureCommand,
  'reiniciar': handleRestartCommand,
...
let recognition = initVoiceRecognition();

// Função para ativar/desativar a assistência por voz
function toggleVoiceAssist() {
  voiceAssistEnabled = !voiceAssistEnabled;

  if (voiceAssistEnabled) {
    voiceBtn.classList.add('active');
    voiceStatus.textContent = 'Ativada';
    startVoiceRecognition();
  } else {
    voiceBtn.classList.remove('active');
    voiceStatus.textContent = 'Desativada';
...
function startVoiceRecognition() {
  if (recognition) {
    recognition.start();
    recognition.onend = () => {
      if (voiceAssistEnabled) recognition.start();
    };
  }
}
...
// Exemplo de funções para lidar com comandos
function handleMoveCommand(command) {
  // Implemente a lógica para mover peças
  console.log('Executando comando de mover:', command);
}

function handleCaptureCommand(command) {
  // Implemente a lógica para capturar peças
  console.log('Executando comando de capturar:', command);
}
...
voiceBtn.addEventListener('click', toggleVoiceAssist);
